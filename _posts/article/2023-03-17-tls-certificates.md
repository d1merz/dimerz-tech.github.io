...
![alt text](../../images/leetcode/tls-certificates/alice_bob_eva.png)
...
Еще немного теории и мы доберемся до разоблачения Евы :smirk_cat:
...
4. Боб, зная только публичный ключ Алисы, поочереди пробуем расшифровать оба хеша и только в одном случае ему это удается
...
и при совпадении убедиться, что сообщение доставлено корректно без изменений. Что же посмотрим как это выглядело бы на практике:
...
![alt text](../../images/leetcode/tls-certificates/cert1.png)
...
![alt text](../../images/leetcode/tls-certificates/cert_exchange.png)

### Полный сценарий
Давайте создадим свой личный PKI и передадим наконец сообщение по самой безопасной схеме на сегодняшний день
...
![alt text](../../images/leetcode/tls-certificates/cert2.png)

Следующий шаг - сгенерировать сертификаты Боба, Алисы и Евы и запросить у CA их подпись, то есть сделать CSR. На самом деле сертификат будет сгенерирован на следующем этапе
удостоверяющим центром. Сейчас мы сгенерируем CSR с необходимыми для выдачи сертификата данными.
```bash
...
Здесь мы так же используем `req`, однако напрямую не говорим, о том, что нужно сгенерировать сертификат, поэтому она генерирует лишь запрос на его подпись.
Здесь есть тонкий и не совсем прозрачный момент, почему мы указываем приватный ключ, если его нельзя никому передавать? 
Все дело в том, что в запросе на подпись сертификата содержится также информация об организации (в нашем случае о частных лицах), а также публичный ключ этой организации.
...
CA знает, о том, что Ева является злоумышленником и не подписывает ее сертификат. Для остальных же выдаем подписанный сертификат:
...
Ева же не может отправить свой сертификат Бобу, либо она может сделать свой CA, подписать им свой сертификат, но ее удостоверяющий центр нарушит свое главное свойство - ему не будут доверять.
