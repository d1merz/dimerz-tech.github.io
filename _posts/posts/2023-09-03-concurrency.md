...
<a id="i4-3"></a>
### Tokio
Выше уже было сказано, что необходим некий рантайм для того, чтобы приложение действительно было асинхронным.
...
Получается, что нельзя обрабатывать входящие соединения от других пользователей в это время. Но на схеме [выше](#anchor) было отмечено, 
что рантайм (`Executor`) обладает некоторой очередью задач и работает асинхронно, то есть никакая задача не блокирует другую. 
...
Приведенный выше пример полностью описывает идею `Concurrency` и является достаточно наглядным для демонстрации всех базовых особенностей.
Для нюансов и тонкостей следует читать документацию по [Tokio](https://tokio.rs/tokio/tutorial). На данный момент я считаю, это лучшая
документация по асинхронности среди всех языков программирования.
<a id="i5"></a>
## Полезные ссылки
